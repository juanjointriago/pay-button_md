...
export const AddEntityForm: FC<Props> = ({ id }) => {
  const [selected, setSelected] = useState<number[]>([]);
  const [show, setShow] = useState(false);
  const dropdownRef = useRef<any>(null);
  const [options, setOptions] = useState<Option[]>([]);
  const trigger = useRef<any>(null);
  const getEntities = useEntitiesStore((state) => state.getAndSetEntities);
  const entities = useEntitiesStore((state) => state.entities);
  const activateEntity = useEntitiesStore((state) => state.activeEntity);
  const inactiveEntity = useEntitiesStore((state) => state.inactiveEntity);
  ...
  const handleActivateEntity = async (entity: Entity) => {
    console.log("💩Entidad seleccionada =>", { entity });
    try {
      if (entity.has_audited === 1) {
        await inactiveEntity(entity.table_name);
        console.log("💩inactivando", entity.table_name);
      } else if (entity.has_audited === 0) {
        await activateEntity(entity.table_name);
        console.log("activando", entity.table_name);
      }
    } catch (error) {
      console.warn({ error });
      Swal.fire({
        title: "Error",
        text: `La entidad ${entity.table_name} no pudo ser ${
          !!entity.has_audited ? "agregada" : "retirada"
        } a la lista de auditadas `,
        icon: "error",
        ...
        <>
          ...
          <form
            onSubmit={(e) => {
              ...
              selected.forEach(async (item) => {
                await handleActivateEntity(
                  entities.find(
                    (entity) => entity.table_name === options[item].value
                  ) as Entity
                );
              });
              setSelected([]);
              setIsLoading(false);
              setMessage(`Cambios guardados de forma exitosa`);
              Swal.fire({
                title: "Entidades",
          ...
          >
            <label className="mb-3 block text-sm font-medium text-black dark:text-white">
              Seleccione entidades para auditar
            </label>
            <div>
              ...
              {selected.length > 0 && (
                <div className="mb-5">
                  <input
                    ...
                    type="submit"
                    value="Auditar entidad seleccionada"
